...
import { ArrowUpRight, Building2, Handshake, LucideIcon, Package, Users2, Zap } from 'lucide-react';
...
const ActionItem = memo(({ 
  ...
  const itemContent = (
    <div className={`p-3 border border-gray-200 dark:border-gray-700 rounded-lg hover:shadow-md transition-all duration-300 group ${className}`}>
      <div className="flex items-center gap-3">
        <div className={`w-10 h-10 rounded-lg bg-gradient-to-br ${gradientColors} flex items-center justify-center shadow-md group-hover:shadow-lg transition-shadow`}>
          <Icon className="w-5 h-5 text-white" />
        </div>
        <div className="flex-1 min-w-0">
          <h3 className="font-medium text-gray-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors text-sm">{title}</h3>
...
const QuickActions = memo(() => {
  ...
  const handleOpenContactModal = useCallback(() => setOpenContactModal(true), []);
  ...
  return (
    <>
      ...
      <DealDrawer  />
      <AddNewContactModal open={openContactModal} onClose={handleDrawerClose} showExtraFields onSave={handleSaveContact} requireHelthcareId/>
    </>
  );
});
